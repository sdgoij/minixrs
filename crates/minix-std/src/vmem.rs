//! VM memory + shared-memory IPC wrappers.
//!
//! `mmap`/`munmap` and the `PROT_*`/`MAP_*` constants live in `minix-rt`
//! (so the runtime's own allocator can map chunks) and are re-exported
//! here; this module keeps the VM stats queries and the shared-memory
//! (IPC server) protocol.
...
pub use minix_rt::vmem::{
    ...
    PROT_WRITE, VM_MMAP, VM_MUNMAP, mmap, munmap,
};
...
mod tests {
    ...
    fn test_vm_call_numbers() {
        ...
        assert_eq!(VM_MAP_PHYS, 0xC0F);
        assert_eq!(VM_INFO, 0xC28);
    }
