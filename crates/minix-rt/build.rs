...
//!
//! The entry/panic/allocator pieces are additionally gated on the `rt`
//! cargo feature (default on): when the crate is used as a dependency of
//! `std` (default-features = false), `std` provides its own `_start`, panic
//! runtime and global allocator, so duplicating them here would be a link
//! error.
