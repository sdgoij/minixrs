...
pub fn unlink(path: &[u8]) -> i64 {
    unsafe { syscall2(NR_UNLINK, path.as_ptr() as u64, path.len() as u64) }
}

/// Remove a directory.
pub fn rmdir(path: &[u8]) -> i64 {
    unsafe { syscall2(NR_RMDIR, path.as_ptr() as u64, path.len() as u64) }
}
...
#[cfg(all(not(test), target_os = "minix", feature = "rt", target_arch = "x86_64"))]
#[unsafe(no_mangle)]
#[unsafe(naked)]
...
#[cfg(all(not(test), target_os = "minix", feature = "rt"))]
#[panic_handler]
