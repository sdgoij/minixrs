...
test-qemu: build
    @set RUSTFLAGS=-C link-arg=-Ttools/minix-raw.ld && {{RUSTUP}} run nightly cargo build -p kernel-boot --target {{TARGET}} -Zjson-target-spec -Zbuild-std=core,alloc --features embed_initramfs,integration-tests --release
