...
fn main() {
    ...
    // Generate Rust source with embedded bytes
    let rs_path = target_dir.join("minixfs_data.rs");
    let mut rs = String::new();
    rs.push_str("// Auto-generated by tools/mkminixfs.rs\n");
    rs.push_str("#[allow(dead_code)]\n");
    rs.push_str("#[allow(unused_attributes)]\n");
    rs.push_str("#[unsafe(link_section = \".minixfs\")]\n");
    rs.push_str("#[used]\n");
    rs.push_str("pub static MINIXFS_IMG: [u8; ");
    rs.push_str(&image.len().to_string());
    rs.push_str("] = [\n    ");
    ...
    rs.push_str("\n];\n");
    rs.push_str("pub const MINIXFS_IMG_LEN: usize = ");
    rs.push_str(&image.len().to_string());
    rs.push_str(";\n");
