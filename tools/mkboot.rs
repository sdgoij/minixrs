...
fn main() {
    ...
    // 3. objcopy the kernel to a raw binary. This happens before the trampoline
    ...
    // 4. Build the trampoline with the correct address.
    ...
    // The kernel image rides along as a second object: a stub that `.incbin`s
    // the raw binary into the `.kernel` output section, which `trampoline.ld`
    // places at 0x200000.
    let blob_s = workspace.join("target").join(format!("{stem}-kernel.S"));
    let blob_o = workspace.join("target").join(format!("{stem}-kernel.o"));
    std::fs::write(
        &blob_s,
        format!(
            "/* Generated by tools/mkboot.rs - the kernel image as a loadable section. */\n\
             .section .kernel, \"a\"\n\
             .incbin \"{}\"\n",
            kernel_bin.to_string_lossy().replace('\\', "/")
        ),
    )
    ...
    std::fs::remove_file(&blob_o).ok();
    std::fs::remove_file(&blob_s).ok();
